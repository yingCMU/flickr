package com.quintostdio.test.flickr.data;
...
import com.quintostdio.test.flickr.ui.FlickrActivity;
import com.quintostdio.test.flickr.utils.Internet;
import com.quintostdio.test.flickr.utils.Parser;
import com.quintostdio.test.flickr.utils.RemoteImage;
import com.quintostdio.test.flickr.utils.Screen;
	...
	protected void onPostExecute(Boolean result) {
		...
		if(result){
			try {
				...
				LinearLayout llTaPantaOla = (LinearLayout) ((Activity)context).findViewById(R.id.llTaPantaOla);
				
				JSONArray  Categories=new JSONObject(thePhotos).getJSONObject("photosets").getJSONArray("photoset");
				
				ArrayList<PhotoSet> thesets = new ArrayList<PhotoSet>();
				
				for (int i=0;i<Categories.length();i++){
					PhotoSet p = new PhotoSet();
					p.title =Categories.getJSONObject(i).getJSONObject("title").getString("_content");
					p.id=Categories.getJSONObject(i).getString("id");
					p.thumb="http://farm"+Categories.getJSONObject(i).getString("farm")+".static.flickr.com/"+
							Categories.getJSONObject(i).getString("server")+"/"+
							Categories.getJSONObject(i).getString("primary")+"_"+
							Categories.getJSONObject(i).getString("secret")+"_s.jpg";
					thesets.add(p);
				}
				...
				for (int i=0;i<thesets.size();i++){
					...
					ivImage.setOnClickListener(new galleryClick(thesets.get(i).id));					
